  ...
  return (
    <svg viewBox={`0 0 ${width} ${h}`} className="w-full h-auto">
      ...
      {items.map((it, idx) => {
        ...
        const heightVal = (safeVal / maxVal) * innerH
        const y = padding.top + innerH - heightVal
        const color = COLORS[it.protocolId] || '#94a3b8'
        return (
          <g key={it.protocolId} style={{ mixBlendMode: 'screen' as any }}>
            <rect x={x} y={y} width={barWidth} height={heightVal} fill="none" stroke={color} strokeWidth={6} opacity={0.25} filter="url(#glow)">
              <animate attributeName="opacity" values="0.22;0.28;0.22" dur="5s" repeatCount="indefinite" />
            </rect>
            <rect x={x} y={y} width={barWidth} height={heightVal} fill={color} opacity={0.92} />
            <rect x={x + 0.25} y={y + 0.25} width={barWidth - 0.5} height={Math.max(heightVal - 0.5, 0)} fill="none" stroke="#0b0f1a" strokeOpacity={0.28} strokeWidth={0.75} />
