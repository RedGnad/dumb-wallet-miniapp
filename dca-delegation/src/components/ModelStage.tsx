...
function FitToUnit({
  ...
  useEffect(() => {
    ...
    if (variant === "conservative")
      baseScale = 1.6; // ENCORE plus grande pour conservative
...
function FallbackMesh({
  ...
  const geo = (() => {
    switch (variant) {
      case "conservative":
        return <icosahedronGeometry args={[1, 1]} />; // Boule pour conservative
      case "balanced":
        return <torusGeometry args={[0.9, 0.28, 16, 64]} />;
      case "aggressive":
        return <octahedronGeometry args={[1.4, 0]} />; // RAMIEL EVANGELION pour aggressive
      case "contrarian":
  ...
  const color =
    variant === "conservative"
      ? "#7c3aed" // VIOLETTE pour conservative
      : variant === "balanced"
      ? "#22c55e"
      : variant === "aggressive"
      ? "#3b82f6" // BLEU CLAIR IRISÉ RAMIEL pour aggressive
      : variant === "contrarian"
      ? "#0ea5e9"
      : "#7c3aed";
  return (
...
function getVariantFromUrl(
  url: string
): "conservative" | "balanced" | "aggressive" | "contrarian" | "default" {
    ...
    <div className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 w-[560px] h-[560px]">
      ...
      >
        <group ref={root}>
          ...
          {fallbackVariant === "aggressive" ||
          fallbackVariant === "conservative" ? (
