...
function setBigUint64(view, byteOffset, value, isLE5) {
  ...
  const h2 = isLE5 ? 4 : 0;
  const l2 = isLE5 ? 0 : 4;
  view.setUint32(byteOffset + h2, wh, isLE5);
  view.setUint32(byteOffset + l2, wl, isLE5);
}
function Chi(a, b2, c) {
  return a & b2 ^ ~a & c;
}
...
var SHA1 = class extends HashMD {
  ...
  process(view, offset) {
    ...
    for (let i2 = 0; i2 < 80; i2++) {
      ...
      const T2 = rotl(A, 5) + F + E + K3 + SHA1_W[i2] | 0;
      E = D;
      D = C;
      C = rotl(B2, 30);
      B2 = A;
...
var SHA12 = class extends HashMD3 {
  ...
  process(view, offset) {
    for (let i2 = 0; i2 < 16; i2++, offset += 4)
      SHA1_W2[i2] = view.getUint32(offset, false);
    for (let i2 = 16; i2 < 80; i2++)
      SHA1_W2[i2] = rotl2(SHA1_W2[i2 - 3] ^ SHA1_W2[i2 - 8] ^ SHA1_W2[i2 - 14] ^ SHA1_W2[i2 - 16], 1);
    let { A, B: B2, C, D, E } = this;
    for (let i2 = 0; i2 < 80; i2++) {
      let F, K3;
      if (i2 < 20) {
      ...
      const T2 = rotl2(A, 5) + F + E + K3 + SHA1_W2[i2] | 0;
      E = D;
      D = C;
      C = rotl2(B2, 30);
      B2 = A;
...
var K2 = Array.from({ length: 64 }, (_2, i2) => Math.floor(p322 * Math.abs(Math.sin(i2 + 1))));
...
var MD52 = class extends HashMD3 {
  ...
  process(view, offset) {
    for (let i2 = 0; i2 < 16; i2++, offset += 4)
      MD5_W2[i2] = view.getUint32(offset, true);
    let { A, B: B2, C, D } = this;
    for (let i2 = 0; i2 < 64; i2++) {
      let F, g, s;
      if (i2 < 16) {
      ...
      F = F + A + K2[i2] + MD5_W2[g];
      A = D;
      D = C;
      C = B2;
      B2 = B2 + rotl2(F, s[i2 % 4]);
    }
    A = A + this.A | 0;
    B2 = B2 + this.B | 0;
    C = C + this.C | 0;
...
var Id1602 = (() => Uint8Array.from(new Array(16).fill(0).map((_2, i2) => i2)))();
var Pi1602 = (() => Id1602.map((i2) => (9 * i2 + 5) % 16))();
var idxLR2 = (() => {
  const L = [Id1602];
  const R = [Pi1602];
  const res = [L, R];
...
var shiftsL1602 = idxL2.map((idx, i2) => idx.map((j) => shifts1602[i2][j]));
var shiftsR1602 = idxR2.map((idx, i2) => idx.map((j) => shifts1602[i2][j]));
...
var RIPEMD1602 = class extends HashMD3 {
  ...
  process(view, offset) {
    for (let i2 = 0; i2 < 16; i2++, offset += 4)
      BUF_1602[i2] = view.getUint32(offset, true);
    let al = this.h0 | 0, ar = al, bl = this.h1 | 0, br = bl, cl = this.h2 | 0, cr = cl, dl = this.h3 | 0, dr = dl, el = this.h4 | 0, er = el;
    for (let group = 0; group < 5; group++) {
      const rGroup = 4 - group;
      ...
      for (let i2 = 0; i2 < 16; i2++) {
        const tl = rotl2(al + ripemd_f2(group, bl, cl, dl) + BUF_1602[rl[i2]] + hbl, sl[i2]) + el | 0;
        al = el, el = dl, dl = rotl2(cl, 10) | 0, cl = bl, bl = tl;
      }
      for (let i2 = 0; i2 < 16; i2++) {
        const tr = rotl2(ar + ripemd_f2(rGroup, br, cr, dr) + BUF_1602[rr[i2]] + hbr, sr[i2]) + er | 0;
...
function split3(lst, le = false) {
  ...
  let Al = new Uint32Array(len);
  for (let i2 = 0; i2 < len; i2++) {
    const { h: h2, l: l2 } = fromBig3(lst[i2], le);
    [Ah[i2], Al[i2]] = [h2, l2];
  }
  return [Ah, Al];
}
var shrSH3 = (h2, _l, s) => h2 >>> s;
var shrSL3 = (h2, l2, s) => h2 << 32 - s | l2 >>> s;
var rotrSH3 = (h2, l2, s) => h2 >>> s | l2 << 32 - s;
var rotrSL3 = (h2, l2, s) => h2 << 32 - s | l2 >>> s;
var rotrBH3 = (h2, l2, s) => h2 << 64 - s | l2 >>> s - 32;
var rotrBL3 = (h2, l2, s) => h2 >>> s - 32 | l2 << 64 - s;
...
var SHA5123 = class extends HashMD3 {
  ...
  process(view, offset) {
    ...
    for (let i2 = 16; i2 < 80; i2++) {
      ...
      const SUMl = add4L3(s0l, s1l, SHA512_W_L3[i2 - 7], SHA512_W_L3[i2 - 16]);
      const SUMh = add4H3(SUMl, s0h, s1h, SHA512_W_H3[i2 - 7], SHA512_W_H3[i2 - 16]);
    ...
    for (let i2 = 0; i2 < 80; i2++) {
      ...
      const CHIh = Eh & Fh ^ ~Eh & Gh;
      const CHIl = El & Fl ^ ~El & Gl;
      const T1ll = add5L3(Hl, sigma1l, CHIl, SHA512_Kl3[i2], SHA512_W_L3[i2]);
      const T1h = add5H3(T1ll, Hh, sigma1h, CHIh, SHA512_Kh3[i2], SHA512_W_H3[i2]);
      const T1l = T1ll | 0;
...
var hexes5 = Array.from({ length: 256 }, (_2, i2) => i2.toString(16).padStart(2, "0"));
...
function Chi4(a, b2, c) {
  return a & b2 ^ ~a & c;
}
function Maj4(a, b2, c) {
  return a & b2 ^ a & c ^ b2 & c;
}
...
function split4(lst, le = false) {
  ...
  let Al = new Uint32Array(len);
  for (let i2 = 0; i2 < len; i2++) {
    const { h: h2, l: l2 } = fromBig4(lst[i2], le);
    [Ah[i2], Al[i2]] = [h2, l2];
  }
  return [Ah, Al];
}
var shrSH4 = (h2, _l, s) => h2 >>> s;
var shrSL4 = (h2, l2, s) => h2 << 32 - s | l2 >>> s;
var rotrSH4 = (h2, l2, s) => h2 >>> s | l2 << 32 - s;
var rotrSL4 = (h2, l2, s) => h2 << 32 - s | l2 >>> s;
var rotrBH4 = (h2, l2, s) => h2 << 64 - s | l2 >>> s - 32;
var rotrBL4 = (h2, l2, s) => h2 >>> s - 32 | l2 << 64 - s;
...
var SHA5124 = class extends HashMD4 {
  ...
  process(view, offset) {
    for (let i2 = 0; i2 < 16; i2++, offset += 4) {
      SHA512_W_H4[i2] = view.getUint32(offset);
      SHA512_W_L4[i2] = view.getUint32(offset += 4);
    }
    for (let i2 = 16; i2 < 80; i2++) {
      const W15h = SHA512_W_H4[i2 - 15] | 0;
      const W15l = SHA512_W_L4[i2 - 15] | 0;
      ...
      const W2h = SHA512_W_H4[i2 - 2] | 0;
      const W2l = SHA512_W_L4[i2 - 2] | 0;
      ...
      const SUMl = add4L4(s0l, s1l, SHA512_W_L4[i2 - 7], SHA512_W_L4[i2 - 16]);
      const SUMh = add4H4(SUMl, s0h, s1h, SHA512_W_H4[i2 - 7], SHA512_W_H4[i2 - 16]);
      SHA512_W_H4[i2] = SUMh | 0;
      SHA512_W_L4[i2] = SUMl | 0;
    }
    let { Ah, Al, Bh, Bl, Ch, Cl, Dh, Dl, Eh, El, Fh, Fl, Gh, Gl, Hh, Hl } = this;
    for (let i2 = 0; i2 < 80; i2++) {
      ...
      const CHIh = Eh & Fh ^ ~Eh & Gh;
      const CHIl = El & Fl ^ ~El & Gl;
      const T1ll = add5L4(Hl, sigma1l, CHIl, SHA512_Kl4[i2], SHA512_W_L4[i2]);
      const T1h = add5H4(T1ll, Hh, sigma1h, CHIh, SHA512_Kh4[i2], SHA512_W_H4[i2]);
      const T1l = T1ll | 0;
...
*/
//# sourceMappingURL=dist-42R6NNXU.js.map
