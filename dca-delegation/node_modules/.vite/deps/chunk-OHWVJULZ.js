...
async function createWebAuthnCredential(parameters) {
  ...
  return {
    id: credential.id,
    publicKey: toHex(credential.publicKey, { includePrefix: false }),
    raw: credential.raw
  };
}
...
async function toCoinbaseSmartAccount(parameters) {
  ...
  const owners_bytes = owners.map((owner2) => {
    if (typeof owner2 === "string")
      return pad(owner2);
    if (owner2.type === "webAuthn")
      return owner2.publicKey;
    if (owner2.type === "local")
      return pad(owner2.address);
    throw new BaseError("invalid owner type");
  });
  const owner = (() => {
    const owner2 = owners[ownerIndex] ?? owners[0];
    if (typeof owner2 === "string")
      return { address: owner2, type: "address" };
    return owner2;
  })();
  return toSmartAccount({
    ...
    async sign(parameters2) {
      ...
      const signature = await signTypedData2({ owner, typedData });
      return wrapSignature({
        ownerIndex,
        signature
      });
    ...
    async signUserOperation(parameters2) {
      ...
      if (owner.type === "address")
        throw new Error("owner cannot sign");
      const signature = await sign2({ hash: hash2, owner });
      return wrapSignature({
        ownerIndex,
        signature
      });
    ...
    userOperation: {
      async estimateGas(userOperation) {
        if (owner.type !== "webAuthn")
          return;
        return {
          verificationGasLimit: BigInt(Math.max(Number(userOperation.verificationGasLimit ?? 0n), 8e5))
        };
      }
    }
  });
}
async function signTypedData2({ typedData, owner }) {
  if (owner.type === "local" && owner.signTypedData)
    return owner.signTypedData(typedData);
  const hash2 = hashTypedData(typedData);
  return sign2({ hash: hash2, owner });
}
async function sign2({ hash: hash2, owner }) {
  if (owner.type === "webAuthn") {
...
function toReplaySafeTypedData({ address, chainId, hash: hash2 }) {
  return {
    domain: {
      chainId,
...
async function signMessage2(client, parameters) {
...
async function signTypedData3(client, parameters) {
  const { account: account_ = client.account, domain, factory, factoryData, message, primaryType, types: types2, verifier } = parameters;
  if (!account_)
    throw new AccountNotFoundError({
...
async function toSoladySmartAccount(parameters) {
  const { address, client, entryPoint: entryPoint_ = {
    ...
    version: "0.7"
  }, factoryAddress: factoryAddress2 = "0x5d82735936c6Cd5DE57cC3c1A799f6B2E6F933Df", getNonce, salt = "0x0" } = parameters;
  const entryPoint = {
    abi: entryPoint_.abi,
  ...
  return toSmartAccount({
    ...
    async signMessage(parameters2) {
      const { message } = parameters2;
      const [address2, { factory: factory2, factoryData }] = await Promise.all([
        this.getAddress(),
        this.getFactoryArgs()
      ]);
      return await signMessage2(client, {
        account: owner,
        factory: factory2,
        factoryData,
        message,
    ...
    async signTypedData(parameters2) {
      ...
      return await signTypedData3(client, {
        account: owner,
        domain,
        message,
        factory: factory2,
    ...
    async signUserOperation(parameters2) {
      ...
      const signature = await getAction(client, signMessage, "signMessage")({
        account: owner,
        message: {
          raw: userOpHash
        }
...
function toWebAuthnAccount(parameters) {
  ...
  return {
    ...
    async signTypedData(parameters2) {
      return this.sign({ hash: hashTypedData(parameters2) });
    },
    type: "webAuthn"
  };
}
...
function getContractError(parameters) {
  ...
  const { abi: abi4, functionName, args, to } = (() => {
...
var entryPoint08Address2 = "0x4337084D9E255Ff0702461CF8895CE9E3b5Ff108";
...
export {
  ...
  entryPoint08Address2 as entryPoint08Address,
  toUserOperation
};
//# sourceMappingURL=chunk-OHWVJULZ.js.map
